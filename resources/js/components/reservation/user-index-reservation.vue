<template>
	<v-row class="justify-center align-center">
		<v-col cols="14" lg="8" ma-2 md="10" sm="14" xl="7">
					...
					<template slot="item" slot-scope="props">
						<tr>
							...
							<td class="text-xs-left">
								<v-icon @click="show(props.item.reservation.id)" small>visibility</v-icon>
								<v-icon @click="cancelReservation(props.item.reservation.id)" small>delete</v-icon>
							</td>
						</tr>
					</template>
				</v-data-table>
...
<script>
  ...
  export default {
    ...
    created() {
      Echo.channel("reservation").listen("ReservationChanged", e => {
        this.getReservations();
      });
    },
    methods: {
      getReservations() {
        axios
          .get(route("api.reservation.customerIndex"))
          .then(response => {
      ...
      cancelReservation(id) {
        axios
          .delete(route("api.reservation.delete", id))
          .then(response => {
